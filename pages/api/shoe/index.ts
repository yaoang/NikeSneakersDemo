...
const bookTypes = Object.keys(ShoeType);
...
async function getBookList(req: NextApiRequest) {
  ...
  const shoes: any[] = await prisma.shoes.findMany({
    ...query,
    // include: {
    //   authors: {
    //     select: {
  ...
  const shoeIds = shoes.map((b) => b.id);
  ...
  for (const rating of bookAverageRatings) {
    const index = shoes.findIndex((b) => b.id === rating.shoeId);
    shoes[index].averageRating = rating._avg.score;
  }
  ...
  for (const rating of bookCountRatings) {
    const index = shoes.findIndex((b) => b.id === rating.shoeId);
    shoes[index].ratings = rating._count.shoeId;
  }

  // Counting.
  const total = await prisma.shoes.count(parseBookListQuery(req.query));
